...
import ruizi.ExchangeRateCalculator.Api;
...
public class App extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		Api apiInitial = new Api(rawName);
		 ...
		 * for (int j = 0; j <10; j++) {
		 * 
		 * // get the string of this button String buttonValue =
		 * numberButtons.get(j).getText(); // if user want to clear if (buttonValue ==
		 * "C") { makeClearButton(numberButtons.get(j));} // set the event and calculate
		 * the value now makeNumericButton(buttonValue, numberButtons.get(j)); // apply
		 * the value to rawCurrency rawCurrency.setText(value.toString()); String txt =
		 * rawCurrency.getText(); Double num = Double.parseDouble(txt); num = num*3.14;
		 * txt = num.toString(); targetCurrency.setText(txt); }
		 */
		// TODO Shun
	 ...
	 * 
	 * @Override public void handle(ActionEvent actionEvent) { value = value * 10 +
	 * Double.parseDouble(s); value = Double.parseDouble(s);
	...
	private void makeClearButton(Button button) {
		 ...
		 * button.setOnAction(new EventHandler<ActionEvent>() {
		 * 
		 * @Override public void handle(ActionEvent actionEvent) { value = (double) 0;
		 * System.out.print(value); } });
		 */
